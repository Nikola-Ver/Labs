...
#define AMOUNTOFTHREADS 4

struct SortTask //Это все индексы
{
    int From;
    int To;
};
...
public:
    ...
    SortTask GetTask()
    {
        ...
        ReleaseMutex(mutex);
        return task;            //Если возвращает -1 значит извлекать нечего
    }
...
void SortMerge(std::string*, long);
void SortPart(long, long, std::string*);
...
int ThreadMain(ThreadParameters* parameters)
{
    ...
    while (*(params.stopFlag) == 0)
    {
        ...
        if (task.From != -1)
        {
            SortPart(task.From, task.To, params.stringMassive);
        }
        else
        {
            Sleep(100);
...
int main()
{
    ...
    HFILE fileToRead = OpenFile("Read.txt", fileInfo, OF_READ);
    if (fileToRead == NULL)
    {
    ...
    SortMerge(stringMassive, amountOfStrings);

    HANDLE fileToWrite = CreateFile(L"Write.txt", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
void SortMerge(std::string* arr, long size)
{
    if (size > 1)
    {
        ...
        SortMerge(arr, leftSize);
        SortMerge(&arr[leftSize], rightSize);
...
void SortPart(long from, long to, std::string* arr)
{
    for (int startIndex = from; startIndex <= to; ++startIndex)
    {
        int smallestIndex = startIndex;

        for (int currentIndex = startIndex; currentIndex <= to; ++currentIndex)
        {
