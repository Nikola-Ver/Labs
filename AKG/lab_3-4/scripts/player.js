...
export class Player {
  ...
  update(delta) {
    ...
    this.pos.x +=
      (Math.cos((this.rot.y * Math.PI) / 180.0) * ax +
        Math.sin((this.rot.y * Math.PI) / 180.0) * az) *
      this.speed *
      delta;
    this.pos.z +=
      (-Math.sin((this.rot.y * Math.PI) / 180.0) * ax +
        Math.cos((this.rot.y * Math.PI) / 180.0) * az) *
      this.speed *
      delta;
    ...
    this.cameraTransform = this.cameraTransform.translate(
      -this.pos.x,
      -this.pos.y,
      -this.pos.z
    );
